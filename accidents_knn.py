...
def kernelKNN(x, X, Y, nn):
	d = LA.norm(x -X, axis=1) #distancia de cada muestra 
	idx = np.argsort(d); #indice que ordena las muestras
	y_hat , _ = stats.mode(Y[idx[0:nn]]) #cuál es el más frecuente nn->entre cuantos compara
	return y_hat[0]
